...
skip_layers = ['fc8'] #['conv1','conv2','conv3','conv4','conv5']
train_layers = ['pool1','pool2','pool3','pool4','pool5','norm1','norm2','conv1','conv2','conv3','conv4','conv5','fc6','fc7','graph-dense','concatenation', 'tag-dense','dense1', 'dense3']#['graph-dense', 'graph-drop', 'tag-dense', 'tag-drop', 'concat', 'dense1', 'dense2', 'sig']
# train_layers = ['concatenation','graph-dense','dense3']
...
for learning_rate in learning_rates:
    ...
    x2 = tf.placeholder(tf.float32, [batch_size, 128])
    ...
    graph_output = tf.layers.dense(inputs=x2, units=2048, activation=tf.nn.relu, name = 'graph-dense')
    graph_output = tf.layers.dropout(inputs=graph_output, rate=0.5)
    
    concatenation = tf.concat([alexnet_output, graph_output], 1, name = 'concat')
    ...
    with tf.Session() as sess:
        ...
        for _ in range(test_batches_per_epoch):
            #print "beginning index:",X_test_generator.start
    ...
    file = open('output/ourFramework.txt', 'w')
